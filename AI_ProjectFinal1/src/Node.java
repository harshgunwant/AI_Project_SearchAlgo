...
public class Node implements Comparable<Node> {
    ...
    public Node(Node parent, int[][] puzzle, int x, int y, int newX, int newY, int cost, boolean useHeuristic, boolean ManhattanUsed) {
        // Assign parent node
        this.parent = parent;
        ...
        if (useHeuristic) {
            this.costs += calculateHeuristic(this.puzzle, ManhattanUsed);
        }
    }

    /// Calculate the heuristic value based on the puzzle and Manhattan Distance flag
    public int calculateHeuristic(int[][] puzzle, boolean ManhattanUsed) {
        if (EightPuzzle.isGoal(puzzle)) {
            return 0; // Heuristic is zero for the goal state
        } else if (ManhattanUsed) {
